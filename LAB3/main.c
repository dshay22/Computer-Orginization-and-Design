...
void everything(int* corrdArray, int *frame, int *window, int* aSize){
    ...
    printf("%d\n", (aSize[1] - aSize[3]));
    printf("%d, %d\n", col, row);
    for(int i = 1; i < 16 * 16/*((aSize[0] - aSize[2]) * (aSize[1] - aSize[3]))*/; ++i){
        ...
        if((row == (aSize[0] - aSize[2])) && (col == aSize[1] - aSize[3])){
            break;
        }
...
int main() {
    ...
    int y;
    int coord[] = {-1, -1};
    int asize[] = {8, 16, 4, 4}; //rows , colomns 
    ...
    // everything(&coord, frame4, window4, asize);
    // printf("<%d, %d>\n", coord[0], coord[1]);

    everything(coord, frame5, window5, asize);  //works when aSize[16,16,4,4]
    printf("<%d, %d>\n", coord[0], coord[1]);
